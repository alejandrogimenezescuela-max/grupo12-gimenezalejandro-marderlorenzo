@extends("plantilla")
@section('title', 'Perfil')
@section('content')

<div class="login-wrapper">
    <div class="container">
        <div class="row align-items-center">

            <div class="col-md-6 d-none d-md-block px-5">
                <div class="welcome-text">
                    <h1>¡Hola de nuevo!</h1>
                    <p class="lead">
                        Entrená tu mente, potenciá tu cuerpo.<br>
                        Entrá a tu cuenta de TatamiHUB para ver tus pedidos y suplementos favoritos.
                    </p>
                    <img src="{{ asset('img/miscalenea/logoDragon.png') }}" alt="Logo" class="me-3" style="height: 60px; width: auto;">
                </div>
            </div>

            <div class="col-md-6 d-flex justify-content-center">
                {{-- 1. CONECTAMOS EL FORMULARIO CON LA RUTA --}}
                <form class="form" method="POST" action="/login">
                    @csrf {{-- 2. TOKEN DE SEGURIDAD OBLIGATORIO --}}

                    <h2 class="text-center mb-4" style="font-weight: 800;">INICIAR SESIÓN</h2>

                    {{-- BLOQUE PARA MOSTRAR ERRORES EN CASO DE LOGEO FALLIDO --}}
                    @if ($errors->any())
                        <div class="alert alert-danger py-2" style="font-size: 14px; border-radius: 8px;">
                            <ul class="mb-0 ps-3">
                                @foreach ($errors->all() as $error)
                                    <li>{{ $error }}</li>
                                @endforeach
                            </ul>
                        </div>
                    @endif

                    {{-- MENSAJE DE ÉXITO CUANDO VIENE DESDE EL REGISTRO --}}
                    @if (session('success'))
                        <div class="alert alert-success py-2" style="font-size: 14px; border-radius: 8px;">
                            {{ session('success') }}
                        </div>
                    @endif

                    <div class="flex-column">
                        <label>Email</label>
                    </div>
                    <div class="inputForm">
                        <svg height="20" viewBox="0 0 32 32" width="20" xmlns="http://www.w3.org/2000/svg"><path d="m30.853 13.87a15 15 0 0 0 -29.729 4.082 15.1 15.1 0 0 0 12.876 12.918 15.6 15.6 0 0 0 2.016.13 14.85 14.85 0 0 0 7.715-2.145 1 1 0 1 0 -1.031-1.711 13.007 13.007 0 1 1 5.458-6.529 2.149 2.149 0 0 1 -4.158-.759v-10.856a1 1 0 0 0 -2 0v1.726a8 8 0 1 0 .2 10.325 4.135 4.135 0 0 0 7.83.274 15.2 15.2 0 0 0 .823-7.455zm-14.853 8.13a6 6 0 1 1 6-6 6.006 6.006 0 0 1 -6 6z"></path></svg>
                        {{-- 3. AGREGAMOS EL ATRIBUTO name="email" --}}
                        <input type="email" name="email" class="input" placeholder="Ingrese su Email" value="{{ old('email') }}" required>
                    </div>

                    <div class="flex-column">
                        <label>Contraseña</label>
                    </div>
                    <div class="inputForm">
                        <svg height="20" viewBox="-64 0 512 512" width="20" xmlns="http://www.w3.org/2000/svg"><path d="m336 512h-288c-26.453125 0-48-21.523438-48-48v-224c0-26.476562 21.546875-48 48-48h288c26.453125 0 48 21.523438 48 48v224c0 26.476562-21.546875 48-48 48zm-288-288c-8.8125 0-16 7.167969-16 16v224c0 8.832031 7.1875 16 16 16h288c8.8125 0 16-7.167969 16-16v-224c0-8.832031-7.1875-16-16-16zm0 0"></path><path d="m304 224c-8.832031 0-16-7.167969-16-16v-80c0-52.929688-43.070312-96-96-96s-96 43.070312-96 96v80c0 8.832031-7.167969 16-16 16s-16-7.167969-16-16v-80c0-70.59375 57.40625-128 128-128s128 57.40625 128 128v80c0 8.832031-7.167969 16-16 16zm0 0"></path></svg>
                        {{-- 4. AGREGAMOS EL ATRIBUTO name="password" --}}
                        <input type="password" name="password" class="input" placeholder="Ingrese su Contraseña" required>
                    </div>

                    <div class="flex-row">
                        <div>
                            <input type="checkbox" name="remember">
                            <label style="font-size: 14px;">Recordarme</label>
                        </div>
                        <a href="/olvidaste-contrasena" class="span" style="text-decoration: none;">¿Olvidaste tu contraseña?</a>
                    </div>

                    {{-- Podés cambiar "Sign In" por "Iniciar Sesión" si querés dejar todo en español --}}
                    <button type="submit" class="button-submit">Iniciar Sesión</button>

                    <p class="p">¿No tenés una cuenta? <a href="/register"><span class="span">Crear cuenta</span></a></p>

                    <div class="flex-row">
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
@endsection